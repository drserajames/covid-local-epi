...
lrf_lad_utla_reg <- rbind(lrf_lad_utla_reg, 
                          c("", "Cornwall and Isles of Scilly", "", "Devon, Cornwall & Isle of Scilly", "", "", "Cornwall and Isles of Scilly", "", "South West", ""),
                          c("", "Hackney and City of London", "", "London", "", "", "Hackney and City of London", "", "London", "")
)
...
phe_data <- NULL
uni_ltla <- sort(unique(phe_cases[phe_cases[, "Area type"]=="ltla",1]))
phe_data$ltla <- matrix(0, ncol=length(phe_date), nrow=length(uni_ltla), dimnames=list(uni_ltla, as.character(phe_date)))
for (i in 1:length(uni_ltla)){
  phe_data$ltla[i,as.character(phe_cases[phe_cases[,"Area name"]==uni_ltla[i],"Specimen date"])] <- 
    phe_cases[phe_cases[,"Area name"]==uni_ltla[i],"Daily lab-confirmed cases"]
}
uni_utla <- sort(unique(phe_cases[phe_cases[, "Area type"]=="utla",1]))
...
# ons
# ons <- as.matrix(read_excel("data/lahbtablesweek30.xlsx", sheet="Occurrences - All data", skip=2))
...
# write.csv(all_loc, "data/all_loc.csv")
all_loc <- read.csv("data/all_loc.csv", row.names=1)
week_n <- max(as.numeric(all_loc[, "Group.3"]))
...
for (i in 5:1){
  for (j in 6:1){
  cases[[i]][[j]] <- data_transform(phe_data[[i]], j, ons_pop[[i]])
  }
}
