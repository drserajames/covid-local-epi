...
# plot functions
plot_data <- function(date, data, name=rownames(data), log=F){
  fig <- plot_ly()
...
plot_regional <- function(input, date, data, log=F, src="phe", ons_type=1){
  if (src=="phe"){
    fig2 <- plot_data(date, data[[4]][[as.numeric(input$radio_type)]], log=log)
    fig2 <- plot_title(fig2, input, 3)
    fig2 <- plot_rect(fig2, input, data[[4]][[as.numeric(input$radio_type)]], log=log, src=src)
  }
  if (src=="ons"){
...
plot_lrf <- function(input, date, data, log=F, src="phe", ons_type=1){
  if (src=="phe"){
    if(input$radio_regional==1){
      ...
      fig2 <- plot_data(date, data[[3]][[as.numeric(input$radio_type)]], 
                        name=names(lrf[[as.numeric(input$radio_regional)]][2:length(lrf[[as.numeric(input$radio_regional)]])]), log=log)
      fig2 <- plot_rect(fig2, input, data[[3]][[as.numeric(input$radio_type)]][names(lrf[[as.numeric(input$radio_regional)]][2:length(lrf[[as.numeric(input$radio_regional)]])]),], log=log, src=src)
    }
  ...
  if (src=="phe"){
    if(input$radio_regional==1){
      ...
      if (input_lrf==1){
        ...
        fig2 <- plot_rect(fig2, input, data[[2]][[as.numeric(input$radio_type)]][unique(names(unlist(utla[[as.numeric(input$radio_regional)]])))[2:length(unique(names(unlist(utla[[as.numeric(input$radio_regional)]]))))],], log=log, src=src)
        ...
        fig2 <- plot_rect(fig2, input, data[[2]][[as.numeric(input$radio_type)]][names(utla[[as.numeric(input$radio_regional)]][[as.numeric(input_lrf)]])[2:length(names(utla[[as.numeric(input$radio_regional)]][[as.numeric(input_lrf)]]))],], log=log, src=src)
...
plot_ltla <- function(input, date, data, log=F, src="phe", ons_type=1){
  input_lrf <- input[[paste0("radio_lrf", input$radio_regional)]]
  if (src=="phe"){
    if(input$radio_regional==1){
      ...
      if (input_lrf==1){
        ...
        fig2 <- plot_rect(fig2, input, data[[1]][[as.numeric(input$radio_type)]][name=name_all[2:length(name_all)],], log=log, src=src)
